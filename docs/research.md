  ...
  - [项目背景](#项目背景)
    ...
    - [Rust语言特性](#rust语言特性)
      ...
      - [Rust](#rust)
        ...
        - [Rust的并发风格](#rust的并发风格)
  - [立项依据](#立项依据)
    - [Rust重写FreeRTOS](#rust重写freertos)
    - [Capability安全模型的引入](#capability安全模型的引入)
    - [多核拓展](#多核拓展)
    - [版本迭代](#版本迭代)
  - [前瞻性/重要性分析](#前瞻性重要性分析)
...
## 立项依据
### Rust重写FreeRTOS
``FreeRTOS`` 采用C语言编写，在C诞生的时代，人们倾向手动管理内存，而随着软件大小逐渐变大，工程师疏忽导致的错误操作如空指针、野指针、重复释放、越界访问、数据竞争等会导致严重的内存问题。而我们使用Rust进行代码重构可以避免FreeRTOS的内存隐患。
### Capability安全模型的引入
基于实时操作系统``simplicity``的原则，``FreeRTOS``本质上只是一个线程调度器加上一个TCP/IP协议栈。它没有虚拟内存，没有文件系统，没有安全模型。而FreeRTOS在物联网中应用广泛(aws)，因此我们为FreeRTOS实现一个基于``capability``的security model，提高FreeRTOS面对威胁时的防御力。
...
> 最坏情况执行时间(WCET)是指任务在特定硬件平台上执行的最大时间长度。

虽然FreeRTOS任务调度器使用任务优先级来运行重要的任务，而不是不重要的任务，但如果系统有固有的限制（如处理器的速度），它不能确保它们的最后期限被满足。注意到，一个硬实时系统，如果能够保证它的最后期限被满足，那么它就不会因为增加了处理吞吐量而变得 "更 "实时，尽管它很可能变得更快。因此，在多核架构上实现FreeRTOS的动机并不是要改变它处理任务截止时间的方式或表现出可预测的特性，而是要满足对嵌入式系统不断增长的需求的现实。
### 版本迭代

19级已有用rust改写FreeRTOS的项目`rust-freertos`。经过调研，其改写的是``FreeRTOSv9.0``。而9.0发行于2016年，到现在也经历了五年的发展。现在的FreeRTOS已经更新到``v10.4``版本。对于包括任务间的通信在内的实现有了较大的改进
