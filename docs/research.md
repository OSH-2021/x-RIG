...
本项目以FreeRTOS为重点，主要完成以下4项内容：将seL4的capability机制添加到FreeRTOS中以及使用Rust语言改写FreeRTOS来提高FreeRTOS的安全性，进行FreeRTOS的Rust版本迭代，将FreeRTOS移植到树莓派上运行，通过可加载模块实现FreeRTOS在多核处理器上运行。
